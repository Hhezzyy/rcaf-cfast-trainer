...
def test_disturbance_generator_determinism_same_seed_same_sequence() -> None:
    seed = 123
    g1 = SensoryMotorDisturbanceGenerator(seed=seed)
    g2 = SensoryMotorDisturbanceGenerator(seed=seed)

    seq1 = [g1.next_vector(difficulty=0.65) for _ in range(40)]
    seq2 = [g2.next_vector(difficulty=0.65) for _ in range(40)]
...
def test_score_window_exact_partial_and_zero() -> None:
    ...
    partial = score_window(mean_error=0.33, good_window_error=threshold)
    zero = score_window(mean_error=0.50, good_window_error=threshold)
...
def test_engine_determinism_same_seed_same_control_script() -> None:
    ...
    c1 = FakeClock()
    c2 = FakeClock()
    e1 = build_sensory_motor_apparatus_test(clock=c1, seed=88, difficulty=0.65, config=config)
    e2 = build_sensory_motor_apparatus_test(clock=c2, seed=88, difficulty=0.65, config=config)
    e1.start_scored()
    e2.start_scored()
