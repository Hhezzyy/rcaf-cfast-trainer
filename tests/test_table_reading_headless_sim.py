...
def test_headless_scripted_run_produces_expected_summary_and_scores() -> None:
    ...
    clock.advance(0.2)
    assert engine.submit_answer(str(practice_payload.correct_code)) is True
    ...
    a1 = str(p1_payload.correct_code)
    s1 = scorer.score(problem=p1, user_answer=int(a1), raw=a1)
    ...
    a2 = str(_nearest_wrong_code(p2_payload))
    s2 = scorer.score(problem=p2, user_answer=int(a2), raw=a2)
    ...
    a3_value = p3_payload.correct_value + max(10, p3_payload.estimate_tolerance * 3)
    a3 = str(a3_value)
    s3 = scorer.score(problem=p3, user_answer=int(a3), raw=a3)
