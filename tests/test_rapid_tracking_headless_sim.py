...
def test_headless_scripted_run_produces_expected_summary() -> None:
    ...
    assert summary.capture_max_points >= summary.capture_points
    if summary.capture_max_points > 0:
        assert summary.capture_score_ratio == pytest.approx(
            summary.capture_points / summary.capture_max_points
        )
    else:
