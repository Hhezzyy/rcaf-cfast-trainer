...
def _advance_to_next_prompt_answer_open(
    engine,
    clock: FakeClock,
    *,
    ...
    for _ in range(32):
        ...
        if (
            ...
            and payload.prompt_index != previous_prompt_index
            and payload.trial_stage is TraceTest1TrialStage.ANSWER_OPEN
        ):
            return payload
    raise AssertionError("next TT1 answer-open prompt did not arrive in time")
...
def test_headless_scripted_run_produces_expected_summary() -> None:
    ...
    p2 = _advance_to_next_prompt_answer_open(
        engine,
        clock,
        previous_prompt_index=p1.prompt_index,
    )
    wrong = 1 if int(p2.correct_code) != 1 else 2
    assert engine.submit_answer(str(wrong)) is True

    p3 = _advance_to_next_prompt_answer_open(
        engine,
        clock,
        previous_prompt_index=p2.prompt_index,
    )
